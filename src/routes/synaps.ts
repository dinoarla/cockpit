...
synapsRoutes.get("/work-module-edges", async (c) => {
  const rows = await db.select().from(synapsEdgesTable).where(
    and(eq(synapsEdgesTable.fromType, "work"), eq(synapsEdgesTable.toType, "module"))
  );
...
synapsRoutes.post("/edges", async (c) => {
  ...
  try {
    ...
    if (e?.message?.includes("Duplicate")) return c.json({ error: "Koneksi sudah ada" }, 409);
    throw e;
  }
});
