...
olapTagihanRoutes.get("/summary", async (c) => {
  const rows = await db
    .select({
      ...
      jmlPelanggan:  sql<number>`SUM(jml_pelanggan)`,
      kwhTotal:      sql<number>`SUM(kwh_total)`,
      rpTotal:       sql<number>`SUM(rp_total)`,
      rpPtl:         sql<number>`SUM(rp_ptl)`,
      rpPpj:         sql<number>`SUM(rp_ppj)`,
    })
    .from(olapAmrTagihan)
