...
async function main() {
  ...
  if (!domain) { console.error(`Domain '${domainSlug}' tidak ditemukan.`); process.exit(1); }
  ...
  const [mod] = await db.select().from(domainModules)
    ...
    .limit(1);
  if (!mod) { console.error(`Modul '${moduleSlug}' tidak ditemukan di domain '${domainSlug}'.`); process.exit(1); }
