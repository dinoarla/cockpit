...
import { users, domains, userDomainAccess } from "../db/schema.js";
 ...
 */
export async function requireAuthHtml(c, next) {
    const token = getCookie(c, SESSION_COOKIE_NAME);
    if (!token)
        return c.redirect("/");
    const userId = await validateSessionToken(token);
 ...
 * Pasang SETELAH requireAuth.
 */
export function requireDomainAccess(domainSlug) {
    return async (c, next) => {
        const user = c.get("user");
        if (user.role === "admin")
            return next();
        const [domain] = await db
            ...
            .limit(1);
        if (!domain)
            return c.json({ error: "Domain tidak ditemukan." }, 404);
        const [access] = await db
            ...
            .limit(1);
        if (!access)
            return c.json({ error: "Tidak punya akses ke domain ini." }, 403);
        return next();
    };
}
