...
plnScholarRoutes.get("/search", async (c) => {
    ...
    const conditions = [];
    if (q)
        conditions.push(like(plnScholar.title, `%${q}%`));
    if (entity)
        conditions.push(eq(plnScholar.entity, entity));
    if (bidang)
        conditions.push(eq(plnScholar.bidang, bidang));
    if (unit)
        conditions.push(eq(plnScholar.unit, unit));
    if (year)
        conditions.push(eq(plnScholar.year, parseInt(year)));
...
//# sourceMappingURL=pln-scholar.js.map