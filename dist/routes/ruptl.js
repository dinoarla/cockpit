...
import { ruptlProvinsi, ruptlPenjualanHistoris, ruptlProyeksiKebutuhan, ruptlPembangkitEksisting, ruptlRencanaPembangkit, ruptlRencanaTransmisi, } from "../db/schema.js";
...
ruptlRoutes.get("/provinsi/:kode", async (c) => {
    ...
    const [prov] = await db
        ...
        .limit(1);
    if (!prov)
        return c.json({ error: "Provinsi tidak ditemukan" }, 404);
    return c.json(prov);
});
...
ruptlRoutes.get("/provinsi/:kode/rencana-pembangkit", async (c) => {
    ...
    const rows = await db
        ...
        .where(skenario === "RE_BASE" || skenario === "ARED"
        ? and(eq(ruptlRencanaPembangkit.provinsiId, prov.id), eq(ruptlRencanaPembangkit.skenario, skenario))
...
ruptlRoutes.get("/provinsi/:kode/rencana-transmisi", async (c) => {
    ...
    const rows = await db
        ...
        .where(skenario === "RE_BASE" || skenario === "ARED"
        ? and(eq(ruptlRencanaTransmisi.provinsiId, prov.id), eq(ruptlRencanaTransmisi.skenario, skenario))
...
ruptlRoutes.get("/rencana-pembangkit", async (c) => {
    ...
    const filtered = rows.filter((r) => {
        if (skenario && r.skenario !== skenario)
            return false;
        if (jenis && r.jenis !== jenis)
            return false;
        if (codTahun && String(r.codTahun) !== codTahun)
            return false;
        return true;
    });
    return c.json(filtered);
});
//# sourceMappingURL=ruptl.js.map